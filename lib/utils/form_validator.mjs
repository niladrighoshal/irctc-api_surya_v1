...
async function book_validator(params,class_instance){
    ...
    if(!(Object.prototype.hasOwnProperty.call(params,"date") && typeof params.date === "string" && params.date.length === 10)){
        throw new TypeError("Invalid date parameter: Please use DD-MM-YYYY format.");
    }
    ...
    if(!(Object.prototype.hasOwnProperty.call(params,"mobileNumber") && typeof params.mobileNumber === "string" && params.mobileNumber.length === 10)){
        throw new TypeError("Invalid mobileNumber parameter in journeyDetails");
    }
    if (!(Object.prototype.hasOwnProperty.call(params,"paymentId") && typeof params.paymentId === "string")){
        throw new TypeError("Invalid paymentId parameter in journeyDetails");
    }else{
        if(params.paymentId.length > 3 && params.paymentId.includes("@")){
            params.payment_type = "2"; // UPI
        }
        else if(params.paymentId === "wallet"){
            params.payment_type = "3"; // IRCTC Wallet
        }
        else{
            throw new TypeError("Invalid paymentId parameter. Must be a UPI ID or 'wallet'.");
        }
    }
    let passengerList = [];
    let infantList = [];
    ...
    const new_map = {
        ...
        "autoUpgradationSelected": params.autoUpgradation === true,
        "gnToCkOpted":false,
        ...
        "bookOnlyIfCnf": params.bookOnlyIfConfirm === true,
        "returnJourney":null,
        ...
        "lapAvlRequestDTO":[
            {
                ...
                "autoUpgradation": params.autoUpgradation === true,
                "bookOnlyIfCnf": params.bookOnlyIfConfirm === true,
                "addMealInput":null,
